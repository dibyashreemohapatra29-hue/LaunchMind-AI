...
interface ExecutiveDashboardErrorProps {
  message: string;
  onRetry: () => void;
  onReturnToDashboard: () => void;
}

export function ExecutiveDashboardError({ message, onRetry, onReturnToDashboard }: ExecutiveDashboardErrorProps) {
  return (
    <div className="bg-card border border-border rounded-xl p-8 shadow-sm flex flex-col items-center text-center max-w-lg mx-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
        {/* No retry logic implemented — button simply re-fetches the dashboard data */}
        <button
          onClick={onRetry}
