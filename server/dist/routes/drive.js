...
exports.driveRouter.get("/drive/status", (_req, res) => {
    res.json({ configured: (0, googleAuthService_1.isGoogleDriveConfigured)() });
});
exports.driveRouter.post("/drive/export", async (req, res) => {
    if (!(0, googleAuthService_1.isGoogleDriveConfigured)()) {
        return res.status(503).json({ error: "Google Drive is not configured." });
    }
    const { productName, productType, analysisDate, score, riskLevel, recommendation, executiveSummary, risks, missingRequirements, aiRecommendations, } = req.body ?? {};
    ...
    try {
        const pdfBuffer = await (0, pdfGenerator_1.generatePdfReport)(reportData);
        ...
        const result = await (0, googleDriveService_1.uploadPdfToDrive)(pdfBuffer, fileName);
        res.json({
            success: true,
        ...
        const message = err instanceof Error ? err.message : "Export to Google Drive failed.";
